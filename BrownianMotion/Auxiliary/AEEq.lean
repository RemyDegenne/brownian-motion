...
protected lemma stronglyAdapted (f : α →ₚ[μ, 𝓕] β) : StronglyAdapted 𝓕 f := by
  simp only [cast]
  split_ifs with h
  · exact stronglyAdapted_const 𝓕 _
  · apply AEStronglyAdapted.stronglyAdapted_mk

protected lemma aestronglyAdapted (f : α →ₚ[μ, 𝓕] β) : AEStronglyAdapted f 𝓕 μ :=
...
@[simp]
lemma mk_eq_mk {f g : ι → α → β} {hf hg} : (mk f hf : α →ₚ[μ, 𝓕] β) = mk g hg ↔ f ≡ᵐ[μ] g :=
  Quotient.eq''

@[simp]
lemma mk_coeFn (f : α →ₚ[μ, 𝓕] β) : mk f f.aestronglyAdapted = f := by
...
@[ext]
lemma ext {f g : α →ₚ[μ, 𝓕] β} (h : f ≡ᵐ[μ] g) : f = g := by
  rwa [← f.mk_coeFn, ← g.mk_coeFn, mk_eq_mk]

lemma coeFn_mk (f : ι → α → β) (hf) : (mk f hf : α →ₚ[μ, 𝓕] β) ≡ᵐ[μ] f := by
  rw [← mk_eq_mk (hf := AEEqProcess.aestronglyAdapted ..) (hg := hf), mk_coeFn]

@[elab_as_elim]
lemma induction_on (f : α →ₚ[μ, 𝓕] β) {p : (α →ₚ[μ, 𝓕] β) → Prop} (H : ∀ f hf, p (mk f hf)) :
    p f :=
...
@[simp]
lemma comp_id (f : α →ₚ[μ, 𝓕] β) : comp id (continuous_id) f = f := by
  rcases f; rfl
...
lemma comp_eq_mk (g : β → γ) (hg : Continuous g) (f : α →ₚ[μ, 𝓕] β) :
...
lemma coeFn_comp (g : β → γ) (hg : Continuous g) (f : α →ₚ[μ, 𝓕] β) :
...
lemma pair_eq_mk (f : α →ₚ[μ, 𝓕] β) (g : α →ₚ[μ, 𝓕] γ) :
    f.pair g =
...
lemma coeFn_pair (f : α →ₚ[μ, 𝓕] β) (g : α →ₚ[μ, 𝓕] γ) :
...
lemma comp₂_eq_pair (g : β → γ → δ) (hg : Continuous (uncurry g)) (f₁ : α →ₚ[μ, 𝓕] β)
    (f₂ : α →ₚ[μ, 𝓕] γ) : comp₂ g hg f₁ f₂ = comp _ hg (f₁.pair f₂) :=
  rfl

lemma comp₂_eq_mk (g : β → γ → δ) (hg : Continuous (uncurry g)) (f₁ : α →ₚ[μ, 𝓕] β)
...
lemma coeFn_comp₂ (g : β → γ → δ) (hg : Continuous (uncurry g)) (f₁ : α →ₚ[μ, 𝓕] β)
...
@[simp]
lemma mk_toGerm (f : ι → α → β) (hf) : (mk f hf : α →ₚ[μ, 𝓕] β).toGerm = (fun ω t ↦ f t ω) :=
  rfl

lemma toGerm_eq (f : α →ₚ[μ, 𝓕] β) : f.toGerm = (fun ω t ↦ f t ω) := by
  rw [← mk_toGerm f f.aestronglyAdapted, mk_coeFn]

lemma toGerm_injective : Injective (toGerm : (α →ₚ[μ, 𝓕] β) → Germ (ae μ) (ι → β)) := fun f g H =>
  ext <| EventuallyEq.indist <| Germ.coe_eq.1 <| by rwa [← toGerm_eq, ← toGerm_eq]

lemma comp_toGerm (g : β → γ) (hg : Continuous g) (f : α →ₚ[μ, 𝓕] β) :
...
lemma comp₂_toGerm (g : β → γ → δ) (hg : Continuous (uncurry g)) (f₁ : α →ₚ[μ, 𝓕] β)
    (f₂ : α →ₚ[μ, 𝓕] γ) :
...
lemma liftRel_iff_coeFn {r : (ι → β) → (ι → γ) → Prop} {f : α →ₚ[μ, 𝓕] β} {g : α →ₚ[μ, 𝓕] γ} :
...
lemma coeFn_const (b : β) : (const α b : α →ₚ[μ, 𝓕] β) ≡ᵐ[μ] (fun _ _ ↦ b) :=
  coeFn_mk _ _

/-- If the measure is nonzero, we can strengthen `coeFn_const` to get an equality. -/
@[simp]
lemma coeFn_const_eq [NeZero μ] (b : β) (t : ι) (x : α) : (const α b : α →ₚ[μ, 𝓕] β) t x = b := by
  simp only [cast]
  split_ifs with h
...
lemma coeFn_const_eq' (b : β) : ∃ b', ((const α b : α →ₚ[μ, 𝓕] β) : ι → α → β) = fun _ ↦ b' := by
  simp only [cast]
  split_ifs with h
  case neg => exact h.elim ⟨b, rfl⟩
...
@[to_additive]
lemma one_def [One β] : (1 : α →ₚ[μ, 𝓕] β) = mk (fun _ _ => 1) .const :=
  rfl

@[to_additive]
lemma coeFn_one [One β] : ⇑(1 : α →ₚ[μ, 𝓕] β) ≡ᵐ[μ] 1 :=
  coeFn_const ..

@[to_additive (attr := simp)]
lemma coeFn_one_eq [NeZero μ] [One β] {t : ι} {x : α} : (1 : α →ₚ[μ, 𝓕] β) t x = 1 :=
  coeFn_const_eq ..

@[to_additive (attr := simp)]
lemma one_toGerm [One β] : (1 : α →ₚ[μ, 𝓕] β).toGerm = 1 :=
  rfl
...
lemma coeFn_smul (c : 𝕜) (f : α →ₚ[μ, 𝓕] γ) : ⇑(c • f) ≡ᵐ[μ] c • ⇑f :=
  coeFn_comp _ _ _

lemma smul_toGerm (c : 𝕜) (f : α →ₚ[μ, 𝓕] γ) : (c • f).toGerm = c • f.toGerm :=
  comp_toGerm _ _ _
...
@[to_additive]
lemma coeFn_mul (f g : α →ₚ[μ, 𝓕] γ) : ⇑(f * g) ≡ᵐ[μ] f * g :=
  coeFn_comp₂ _ _ _ _

@[to_additive (attr := simp)]
lemma mul_toGerm (f g : α →ₚ[μ, 𝓕] γ) : (f * g).toGerm = f.toGerm * g.toGerm :=
...
lemma coeFn_pow (f : α →ₚ[μ, 𝓕] γ) (n : ℕ) : ⇑(f ^ n) ≡ᵐ[μ] (⇑f) ^ n :=
  coeFn_comp _ _ _

@[simp]
lemma pow_toGerm (f : α →ₚ[μ, 𝓕] γ) (n : ℕ) : (f ^ n).toGerm = f.toGerm ^ n :=
  comp_toGerm _ _ _
...
@[to_additive]
lemma coeFn_finsetProd [CommMonoid γ] [ContinuousMul γ]
...
@[to_additive]
lemma coeFn_fun_finsetProd [CommMonoid γ] [ContinuousMul γ]
...
@[to_additive (attr := simp)]
lemma inv_mk (f : ι → α → γ) (hf) : (mk f hf : α →ₚ[μ, 𝓕] γ)⁻¹ = mk f⁻¹ hf.inv :=
  rfl

@[to_additive]
lemma coeFn_inv (f : α →ₚ[μ, 𝓕] γ) : ⇑f⁻¹ ≡ᵐ[μ] f⁻¹ :=
  coeFn_comp _ _ _

@[to_additive]
lemma inv_toGerm (f : α →ₚ[μ, 𝓕] γ) : f⁻¹.toGerm = f.toGerm⁻¹ :=
  comp_toGerm _ _ _
...
@[to_additive]
lemma coeFn_div (f g : α →ₚ[μ, 𝓕] γ) : ⇑(f / g) ≡ᵐ[μ] f / g :=
  coeFn_comp₂ _ _ _ _

@[to_additive]
lemma div_toGerm (f g : α →ₚ[μ, 𝓕] γ) : (f / g).toGerm = f.toGerm / g.toGerm :=
  comp₂_toGerm _ _ _ _
...
lemma coeFn_zpow (f : α →ₚ[μ, 𝓕] γ) (n : ℤ) : ⇑(f ^ n) ≡ᵐ[μ] (⇑f) ^ n :=
  coeFn_comp _ _ _

@[simp]
lemma zpow_toGerm (f : α →ₚ[μ, 𝓕] γ) (n : ℤ) : (f ^ n).toGerm = f.toGerm ^ n :=
  comp_toGerm _ _ _
