...
THIS IS A COPY OF THE Egorov FILE WITH `dist` REPLACED BY `edist` (apart from the Aux section)
...
variable {α β ι : Type*} {m : MeasurableSpace α} [EMetricSpace β] {μ : Measure α}

section Aux

@[fun_prop, aesop safe 20 apply (rule_sets := [Measurable])]
protected theorem StronglyMeasurable.edist
    {_ : MeasurableSpace α} {β : Type*} [PseudoEMetricSpace β] {f g : α → β}
...
theorem tendstoUniformlyOn_diff_iUnionNotConvergentSeq (hε : 0 < ε)
    ...
    TendstoUniformlyOn f g atTop
      (s \ EMetricEgorov.iUnionNotConvergentSeq hε hf hg hsm hs hfg) := by
  ...
  obtain ⟨N, hN⟩ : ∃ n : ℕ, 1 / (n + 1 : ℝ≥0∞) < δ := by
    ...
    simp
  rw [eventually_atTop]
  refine ⟨notConvergentSeqLTIndex (half_pos hε) hf hg hsm hs hfg N, fun n hn x hx => ?_⟩
  simp only [Set.mem_diff, iUnionNotConvergentSeq, not_exists, Set.mem_iUnion,
    Set.mem_inter_iff, not_and, exists_and_left] at hx
  obtain ⟨hxs, hx⟩ := hx
  specialize hx hxs N
  rw [mem_notConvergentSeq_iff] at hx
  push_neg at hx
  rw [edist_comm]
...
theorem tendstoUniformlyOn_of_ae_tendsto'' (hf : ∀ n, StronglyMeasurable (f n))
  ...
  ⟨EMetricEgorov.iUnionNotConvergentSeq hε hf hg hsm hs hfg,
    EMetricEgorov.iUnionNotConvergentSeq_subset hε hf hg hsm hs hfg,
    EMetricEgorov.iUnionNotConvergentSeq_measurableSet hε hf hg hsm hs hfg,
    EMetricEgorov.measure_iUnionNotConvergentSeq hε hf hg hsm hs hfg,
    EMetricEgorov.tendstoUniformlyOn_diff_iUnionNotConvergentSeq hε hf hg hsm hs hfg⟩
