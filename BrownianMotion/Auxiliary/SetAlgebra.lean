...
lemma generateHas_def : (generate s).Has = GenerateHas s := rfl
...
-/
lemma generateFrom_eq {s : Set (Set α)} (hs : IsSetAlgebra s) :
...
-/
@[elab_as_elim]
lemma induction_on_monotone {m : MeasurableSpace α} {C : ∀ s : Set α, MeasurableSet s → Prop}
