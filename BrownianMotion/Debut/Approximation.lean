...
variable {𝓕 : Filtration T mΩ} {t : T}
...
def 𝓚₀ (𝓕 : Filtration T mΩ) (t : T) : Set (Set (T × Ω)) :=
  {B | ∃ K C, B = K ×ˢ C ∧ K ⊆ Set.Iic t ∧ IsCompact K ∧ MeasurableSet[𝓕 t] C}

@[simp]
lemma empty_mem_𝓚₀ (𝓕 : Filtration T mΩ) (t : T) : ∅ ∈ 𝓚₀ 𝓕 t := ⟨∅, ∅, by simp⟩

lemma subset_Iic_of_mem_𝓚₀ {B : Set (T × Ω)} (hB : B ∈ 𝓚₀ 𝓕 t) :
...
lemma measurableSet_snd_of_mem_𝓚₀ {B : Set (T × Ω)} (hB : B ∈ 𝓚₀ 𝓕 t) :
    MeasurableSet[𝓕 t] (Prod.snd '' B) := by
...
@[grind .]
lemma inter_mem_𝓚₀ [T2Space T] {𝓕 : Filtration T mΩ} {t : T}
    {B B' : Set (T × Ω)} (hB : B ∈ 𝓚₀ 𝓕 t) (hB' : B' ∈ 𝓚₀ 𝓕 t) : B ∩ B' ∈ 𝓚₀ 𝓕 t := by
...
inductive 𝓚 (𝓕 : Filtration T mΩ) (t : T) : Set (Set (T × Ω)) where
  | base B (hB : B ∈ 𝓚₀ 𝓕 t) : 𝓚 𝓕 t B
  | union B B' (hB : B ∈ 𝓚 𝓕 t) (hB' : B' ∈ 𝓚 𝓕 t) :
      𝓚 𝓕 t (B ∪ B')

lemma 𝓚₀_subset_𝓚 (𝓕 : Filtration T mΩ) (t : T) : 𝓚₀ 𝓕 t ⊆ 𝓚 𝓕 t := fun _ hB ↦ 𝓚.base _ hB

lemma mem_𝓚_iff (𝓕 : Filtration T mΩ) (t : T) (B : Set (T × Ω)) :
    B ∈ 𝓚 𝓕 t ↔ ∃ s : Finset (Set (T × Ω)),
      (s : Set _) ⊆ 𝓚₀ 𝓕 t ∧ B = ⋃ x ∈ s, x := by
  classical
...
@[simp]
lemma empty_mem_𝓚 : ∅ ∈ 𝓚 𝓕 t := 𝓚.base _ (empty_mem_𝓚₀ 𝓕 t)
...
def 𝓚δ (𝓕 : Filtration T mΩ) (t : T) : Set (Set (T × Ω)) :=
  {B | ∃ ℬ ⊆ 𝓚 𝓕 t, ℬ.Nonempty ∧ ℬ.Countable ∧ B = ⋂ b ∈ ℬ, b}

lemma subset_Iic_of_mem_𝓚δ {B : Set (T × Ω)} (hB : B ∈ 𝓚δ 𝓕 t) :
    B ⊆ Set.Iic t ×ˢ .univ := by
  have ⟨ℬ, hℬ_sub, ⟨b, hb⟩, hℬ_count, hB_eq⟩ := hB
  exact hB_eq ▸ Set.iInter₂_subset_of_subset b hb (subset_Iic_of_mem_𝓚 (hℬ_sub hb))

/-- `𝓚(t) ⊆ 𝓚δ(t)`. -/
lemma 𝓚_subset_𝓚δ {B : Set (T × Ω)} (hB : B ∈ 𝓚 𝓕 t) : B ∈ 𝓚δ 𝓕 t :=
  ⟨{B}, fun _ h ↦ h ▸ hB, ⟨B, rfl⟩, (Set.finite_singleton B).countable,
    Eq.symm Set.iInter_iInter_eq_left⟩

@[simp]
lemma empty_mem_𝓚δ : ∅ ∈ 𝓚δ 𝓕 t := 𝓚_subset_𝓚δ (by simp)

/-- `𝓚δ(t)` is closed under union. -/
@[grind .]
lemma union_mem_𝓚δ {𝓕 : Filtration T mΩ} {t : T}
    {B B' : Set (T × Ω)} (hB : B ∈ 𝓚δ 𝓕 t) (hB' : B' ∈ 𝓚δ 𝓕 t) : B ∪ B' ∈ 𝓚δ 𝓕 t := by
  have ⟨ℬ, hℬ_sub, ⟨b, hb⟩, hℬ_count, hB_eq⟩ := hB
  have ⟨ℬ', hℬ_sub', ⟨b', hb'⟩, hℬ_count', hB_eq'⟩ := hB'
...
@[grind .]
lemma inter_mem_𝓚δ {𝓕 : Filtration T mΩ} {t : T}
    {B B' : Set (T × Ω)} (hB : B ∈ 𝓚δ 𝓕 t) (hB' : B' ∈ 𝓚δ 𝓕 t) : B ∩ B' ∈ 𝓚δ 𝓕 t := by
  sorry

/-- `𝓚δ(t)` is closed under countable intersections. -/
lemma iInter_mem_𝓚δ {𝓕 : Filtration T mΩ} {t : T} {B : ℕ → Set (T × Ω)} (hB : ∀ n, B n ∈ 𝓚δ 𝓕 t) :
    (⋂ n, B n) ∈ 𝓚δ 𝓕 t := by
  sorry
...
lemma iInf_snd_eq_snd_iInf_of_mem_𝓚δ {t : T}
    {ℬ : ℕ → Set (T × Ω)} (hℬ : ∀ i, ℬ i ∈ 𝓚δ 𝓕 t) (h_desc : ∀ i, ℬ (i + 1) ⊆ ℬ i) :
...
lemma measurableSet_snd_of_mem_𝓚δ [T2Space T] {B : Set (T × Ω)} (hB : B ∈ 𝓚δ 𝓕 t) :
    MeasurableSet[𝓕 t] (Prod.snd '' B) := by
  ...
  have hG_mem (i : ℕ) : G i ∈ 𝓚 𝓕 t := by
    ...
    | succ i ih =>
      ...
      rw [this]
      grind
  have hG_mem' : ∀ i, G i ∈ 𝓚δ 𝓕 t := fun i ↦ 𝓚_subset_𝓚δ (hG_mem i)
...
`B ∈ 𝓚 𝓕 t`. -/
def 𝓛₀ (X : Type*) [TopologicalSpace X] (𝓕 : Filtration T mΩ) (t : T) : Set (Set (X × (T × Ω))) :=
  {C | ∃ A B, IsCompact A ∧ B ∈ 𝓚 𝓕 t ∧ A ×ˢ B = C}

/-- `𝓛₁(X)` is the collection of finite unions of sets in `𝓛₀(X)`. -/
inductive 𝓛₁ (X : Type*) [TopologicalSpace X] (𝓕 : Filtration T mΩ) (t : T) :
    Set (Set (X × (T × Ω))) where
  | base {L} (hL : L ∈ 𝓛₀ X 𝓕 t) : 𝓛₁ X 𝓕 t L
  | union {L L'} (hL : L ∈ 𝓛₁ X 𝓕 t) (hL' : L' ∈ 𝓛₁ X 𝓕 t) : 𝓛₁ X 𝓕 t (L ∪ L')

/-- `𝓛(X)` is the collection of intersections of countable decreasing sequences in `𝓛₁(X)`. -/
def 𝓛 (X : Type*) [TopologicalSpace X] (𝓕 : Filtration T mΩ) (t : T) : Set (Set (X × (T × Ω))) :=
  {B | ∃ L : ℕ → Set (X × (T × Ω)), Antitone L ∧ (∀ n, L n ∈ 𝓛₁ X 𝓕 t) ∧ B = ⋂ n, L n}

/-- `𝓛σ(X)` is the collection of unions of countable increasing sequences in `𝓛(X)`. -/
def 𝓛σ (X : Type*) [TopologicalSpace X] (𝓕 : Filtration T mΩ) (t : T) : Set (Set (X × (T × Ω))) :=
  {B | ∃ L : ℕ → Set (X × (T × Ω)), Monotone L ∧ (∀ n, L n ∈ 𝓛 X 𝓕 t) ∧ B = ⋃ n, L n}

/-- `𝓛σδ(X)` is the collection of intersections of countable decreasing sequences in `𝓛σ(X)`. -/
def 𝓛σδ (X : Type*) [TopologicalSpace X] (𝓕 : Filtration T mΩ) (t : T) : Set (Set (X × (T × Ω))) :=
  {B | ∃ L : ℕ → Set (X × (T × Ω)), Antitone L ∧ (∀ n, L n ∈ 𝓛σ X 𝓕 t) ∧ B = ⋂ n, L n}
...
`T` and `𝓕 t`, then there exists a compact Hausdorff space `X` and a set `B ∈ 𝓛σδ X 𝓕 t` such that
`A` is the projection of `B` on `T × Ω`. -/
lemma exists_mem_𝓛σδ_of_measurableSet {mT : MeasurableSpace T} [BorelSpace T]
    {A : Set (T × Ω)} (hA_subs : A ⊆ Set.Iic t ×ˢ .univ) (hA : MeasurableSet[mT.prod (𝓕 t)] A) :
    ∃ X : Type*, ∃ _ : TopologicalSpace X, ∃ _ : CompactSpace X, ∃ _ : T2Space X,
      ∃ B ∈ 𝓛σδ X 𝓕 t, A = Prod.snd '' B := by
...
/- does it make sense to have `ε : ℝ≥0∞`, or would it be better to have `ε : ℝ` or `ε : ℝ≥0`?
having `ε : ℝ≥0∞` has the advantage that we dont have coercions inside the formula, but maybe
it does not affect much? for now I leave it as `ℝ≥0∞`, but if it gives any trouble we can
change it. -/
/-- An approximation of `A ⊆ Set.Iic t ×ˢ .univ` is a sequence of sets `B ε ⊆ A` for every `ε > 0`
such that `B ε ∈ 𝓚δ 𝓕 t`, and the projection of `B ε` on `Ω` approximates `A` in measure
as `ε` tends to `0`. -/
structure Approximation (𝓕 : Filtration T mΩ) (P : Measure Ω) (t : T) (A : Set (T × Ω)) where
  /-- The approximating sets. -/
  B : ℝ≥0∞ → Set (T × Ω)
  A_subset : A ⊆ Set.Iic t ×ˢ .univ
  B_mem : ∀ ε > 0, B ε ∈ 𝓚δ 𝓕 t
...
def of_mem_𝓚δ (P : Measure Ω) (hA : A ∈ 𝓚δ 𝓕 t) : Approximation 𝓕 P t A where
  B := fun _ ↦ A
...
def B' (𝒜 : Approximation 𝓕 P t A) : ℕ → Set (T × Ω) :=
  fun n ↦ ⋃ m ∈ Finset.Icc 1 n, 𝒜.B (1 / m)

@[simp]
lemma B'_zero (𝒜 : Approximation 𝓕 P t A) : 𝒜.B' 0 = ∅ := by
  simp [B']

lemma B'_succ (𝒜 : Approximation 𝓕 P t A) (n : ℕ) :
...
lemma le' (𝒜 : Approximation 𝓕 P t A) (n : ℕ) :
    P (Prod.snd '' A) ≤ P (Prod.snd '' 𝒜.B' n) + 1 / n := by
  -- easy, use the definition of B', le and the fact that B (1 / n) ⊆ B' n
  sorry

lemma B'_mono (𝒜 : Approximation 𝓕 P t A) : Monotone 𝒜.B' :=
  monotone_nat_of_le_succ fun n ↦ by simp [B'_succ]

lemma measurableSet_snd_iUnion_B' [T2Space T] (𝒜 : Approximation 𝓕 P t A) :
    MeasurableSet[𝓕 t] (Prod.snd '' (⋃ n, 𝒜.B' n)) := by
...
lemma measure_snd_iUnion_B' (𝒜 : Approximation 𝓕 P t A) :
...
lemma measure_snd_diff [IsFiniteMeasure P] (𝒜 : Approximation 𝓕 P t A) :
...
with respect to `𝓕 t`. -/
lemma measurableSet_snd [T2Space T] (𝒜 : Approximation 𝓕 P t A) :
    MeasurableSet[𝓕 t] (Prod.snd '' A) := by
...
it as in the paper, i.e. `∀ ε > 0, ∃ B ∈ 𝓚δ 𝓕 t, P (Prod.snd '' A \ Prod.snd '' B) < ε` ? In any
case it should be fairly easy to go from one to the other. -/
lemma tendsto_measure_diff_B' (𝒜 : Approximation 𝓕 P t A) :
...
/- Question: Is `A ⊆ Set.Iic t ×ˢ .univ ∧ MeasurableSet[mT.prod (𝓕 t)] A` equivalent to
`A ∈ 𝓑 (Set.Iic t) × (𝓕 i)` (the last one is not syntactically correct in lean, with that I mean
...
-- is there a notation for `mT.prod (𝓕 t)`? `mT × (𝓕 t)` does not work
...
/-- If `A` is measurable with respect to the product sigma algebra of the Borel sigma algebra on
`T` and `𝓕 t`, then it is approximable. -/
def of_mem_prod_borel {mT : MeasurableSpace T} [BorelSpace T] (P : Measure Ω)
    (hA_subs : A ⊆ Set.Iic t ×ˢ .univ) (hA : MeasurableSet[mT.prod (𝓕 t)] A) :
    Approximation 𝓕 P t A where
