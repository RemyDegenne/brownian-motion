...
/-- Formula for the expectation of the elementary stochastic integral of the indicator of an
elementary predictable set, as a sum of set integrals of increments. -/
lemma ElementaryPredictableSet.integral_integral_indicator_one
    (hXint : ∀ s, Integrable (X s) μ) (S : ElementaryPredictableSet 𝓕) (t : ι) :
    μ[(S.indicator (1 : ℝ) ● X) t]
      = ∑ p ∈ S.I, ∫ ω in S.set p, (X (min p.2 t) ω - X (min p.1 t) ω) ∂μ := by
  have h_eq : (S.indicator (1 : ℝ) ● X) t = fun ω ↦ ∑ p ∈ S.I,
      (S.set p).indicator (fun ω ↦ X (min p.2 t) ω - X (min p.1 t) ω) ω := by
    ext ω
  ...
  · intro p hp
    exact ((hXint _).sub (hXint _)).indicator (𝓕.le _ _ (S.measurableSet_set p hp))
