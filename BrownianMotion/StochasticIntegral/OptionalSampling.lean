...
#check Martingale.stoppedValue_ae_eq_condExp_of_le_of_countable_range
...
theorem Martingale.condExp_stoppedValue_stopping_time_ae_eq_restrict_le_of_countable_range
    ...
    (hτ : IsStoppingTime 𝓕 τ) (hτ_countable_range : (Set.range τ).Countable)
    (hσ : IsStoppingTime 𝓕 σ) [SigmaFinite (μ.trim hσ.measurableSpace_le)] :
