...
lemma LinearEquiv {f : F → β} {s : Set F} (hf : StrictConvexOn 𝕜 s f) (g : E ≃ₗ[𝕜] F) :  StrictConvexOn  𝕜 (g ⁻¹' s) (f ∘ g) :=
...
lemma komlos_ennreal (X : ℕ → Ω → ℝ≥0∞) (hX : ∀ n, Measurable (X n))
  ...
  have hZ_integrable {W : Ω → ℝ≥0∞} (hX : Measurable W) : Integrable (Z W) :=
    ⟨(measurable_expInv.comp hX).aestronglyMeasurable, HasFiniteIntegral.of_bounded (hZ_norm_le_one _)⟩
