...
lemma isCadlag_brownian (ω : ℝ≥0 → ℝ) : IsCadlag (brownian · ω) :=
  (continuous_brownian ω).isCadlag
...
lemma martingale_brownian_sq_sub_time :
  ...
  · intro t
    ...
    have hsm : StronglyMeasurable[brownianNaturalFiltration t]
        (fun ω => brownian t ω * brownian t ω - (t : ℝ)) := by
      convert (ht.mul ht).sub
        (stronglyMeasurable_const :
          StronglyMeasurable[brownianNaturalFiltration t]
            (fun _ : ℝ≥0 → ℝ => (t : ℝ))) using 1
      ext ω
      rfl
    simpa [pow_two] using hsm
  · intro s t hst
    haveI : IsFilteredPreBrownian brownian brownianNaturalFiltration gaussianLimit :=
      isBrownianReal_brownian.toIsPreBrownianReal.isFilteredPreBrownian measurable_brownian
    have hM := fun u ↦
    ...
    have hProdCondZero :
      ...
      have hpull := condExp_mul_of_stronglyMeasurable_left hBs_sm hProdInt
        hDeltaInt
      have hDeltaCondZero' :
          gaussianLimit[fun ω => brownian t ω - brownian s ω | brownianNaturalFiltration s]
            =ᵐ[gaussianLimit] fun _ => 0 := by
        convert hDeltaCondZero using 1
        ext ω
        rfl
      have hpull' :
          gaussianLimit[fun ω => brownian s ω * (brownian t ω - brownian s ω) |
    ...
    have hTwoProdCondZero :
      ...
      have hsmul_zero :
          gaussianLimit[(2 : ℝ) •
              (fun ω => brownian s ω * (brownian t ω - brownian s ω)) |
              brownianNaturalFiltration s]
            =ᵐ[gaussianLimit]
              fun _ => 0 := by
    ...
    have hPastSm : StronglyMeasurable[brownianNaturalFiltration s] past := by
      dsimp only [past]
      have hsm : StronglyMeasurable[brownianNaturalFiltration s]
          (fun ω => brownian s ω * brownian s ω - (s : ℝ)) := by
        convert (hBs_sm.mul hBs_sm).sub
          (stronglyMeasurable_const :
            StronglyMeasurable[brownianNaturalFiltration s]
              (fun _ : ℝ≥0 → ℝ => (s : ℝ))) using 1
        ext ω
        rfl
      simpa [pow_two] using hsm
    have hExpand :
...
lemma isCadlag_brownian_sq_sub_time (ω : ℝ≥0 → ℝ) :
  ...
  have htime : IsCadlag (fun t : ℝ≥0 => (t : ℝ)) :=
    (NNReal.continuous_coe : Continuous fun t : ℝ≥0 => (t : ℝ)).isCadlag
  ...
  have hadd : IsCadlag
      ((fun t : ℝ≥0 => brownian t ω ^ 2) +
        fun t : ℝ≥0 => (-1 : ℝ) • (t : ℝ)) :=
    hsq.add hneg
  change IsCadlag ((fun t : ℝ≥0 => brownian t ω ^ 2) + fun t : ℝ≥0 => -(t : ℝ))
  simpa [Pi.add_apply, smul_eq_mul] using hadd
...
lemma locally_isSquareIntegrable_brownian :
    IsLocallySquareIntegrable brownian brownianNaturalFiltration gaussianLimit :=
  Martingale.isLocallySquareIntegrable_of_continuous martingale_brownian continuous_brownian
...
theorem quadraticVariation_brownian (t : ℝ≥0) :
  ...
  have hX2_cadlag : ∀ ω, IsCadlag (X2 · ω) := by
    intro ω
    simpa [X2, Function.comp_def] using IsCadlag.norm_sq (isCadlag_brownian ω)
  have hX_sub : IsLocalSubmartingale X2 brownianNaturalFiltration gaussianLimit := by
    simpa [X2] using
      locally_isSquareIntegrable_brownian.isLocalSubmartingale_sq_norm
