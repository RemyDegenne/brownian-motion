...
lemma lintegral_sup_rpow_edist_le_card_mul_rpow_of_dist_le (hp : 0 < p) (hq : 0 ≤ q)
    ...
    (hJ_card : #J ≤ a ^ n) (ha : 1 < a) (hn : 1 ≤ n) :
    ∫⁻ ω, ⨆ (s) (t) (_hs : s ∈ J) (_ht : t ∈ J) (_hd : edist s t ≤ c),
        edist (X s ω) (X t ω) ^ p ∂P
      ≤ 2 ^ p * a * #J * M * (c * n) ^ q := by
  obtain ⟨K, ⟨-, _, hKeps, hKle⟩⟩ := pair_reduction J hJ_card ha hn E
  calc
...
lemma lintegral_sup_rpow_edist_cover_of_dist_le (hp : 0 < p) (hq : 0 ≤ q)
  ...
  refine (lintegral_sup_rpow_edist_le_card_mul_rpow_of_dist_le hp hq hX h₀'
    (by norm_num) (by omega)).trans ?_
