...
\begin{lemma}\label{lem:upcrossing_nat}
  \uses{def:Submartingale}
  \mathlibok
  \lean{MeasureTheory.Submartingale.mul_integral_upcrossingsBefore_le_integral_pos_part}
Let $X$ be a real submartingale indexed by $\mathbb{N}$.
Then, for every $a < b$, the number of upcrossings $U_t[a,b]$ of the interval $[a,b]$ before time $t$ by $X$ satisfies
\begin{align*}
  (b - a) \mathbb{E}[U_t[a,b]] \le \mathbb{E}[(X_t - a)^+]
  \: .
\end{align*}
\end{lemma}
...
\begin{lemma}\label{lem:upcrossing_countable}
  \uses{def:Submartingale}
Suppose that $T$ is a countable time index with a bottom element $0$.
Let $X$ be a real submartingale on that index set.
Then, for every $a < b$, the number of upcrossings $U_t[a,b]$ of the interval $[a,b]$ before time $t$ by $X$ satisfies
\begin{align*}
  (b - a) \mathbb{E}[U_t[a,b]] \le \mathbb{E}[(X_t - a)^+]
  \: .
...
\begin{proof}
  \uses{lem:upcrossing_nat}
Build an increasing sequence of $\mathbb{N}$-indexed subsets of $T$ whose union is $T$ and apply Doob's upcrossing lemma on each of these subsets (Lemma~\ref{lem:upcrossing_nat}).
Then conclude by monotone convergence.
\end{proof}
