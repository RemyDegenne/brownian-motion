...
\begin{definition}[Approximatable time index]\label{def:approximatableTimeIndex}
  \uses{def:IsStoppingTime, def:approxSeq}
  A time index set \(T\) is said to be approximatable if for any stopping time \(\tau : \Omega \to T \cup \{\infty\}\), there exists a discrete approximation sequence \((\tau_n)\) of \(\tau\).
\end{definition}
...
\begin{lemma}\label{lem:stoppingTime_approximation}
  \uses{def:IsStoppingTime, def:approximatableTimeIndex}
  $T = \mathbb{R}_+$ is an approximatable time index. In particular,
...
\begin{lemma}\label{lem:stoppingTime_approximationNat}
  \uses{def:IsStoppingTime, def:approximatableTimeIndex}
  $T = \mathbb{N}$ is an approximatable time index.
\end{lemma}
\begin{proof}
...
\begin{lemma}[Optional sampling (continuous time)]\label{lem:optionalSampling}
  ...
  Let $X$ be a right-continuous $\mathcal{F}$-martingale on an approximatable time index. Then, for
...
\begin{lemma}\label{lem:optionalSamplingSubmartingale}
  ...
  Let $X$ be a right-continuous $\mathcal{F}$-submartingale on an approximatable time index. Then, for
  any stopping times $\sigma, \tau$ with $\tau$ bounded, we have that
