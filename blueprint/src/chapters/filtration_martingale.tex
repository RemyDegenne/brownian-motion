...
\begin{lemma}\label{lem:optionalSampling_discrete_submartingale}
  ...
  Let $X$ be a discrete time submartingale with respect to the filtration $\mathcal{F}$ taking values in a real Banach space $E$. Assume $E$ is a partial order  Let
  $\tau, \sigma$ be stopping times. Then, if $\tau$ is bounded, we have that almost surely,
  $X_{\tau \wedge \sigma} \le P[X_{\tau} \mid \mathcal{F}_{\sigma}]$.
\end{lemma}
...
\begin{lemma}\label{lem:optionalSampling_discrete_supermartingale}
  \uses{def:Submartingale, def:IsStoppingTime}
  \leanok
  \lean{MeasureTheory.Supermartingale.condExp_ae_le_stoppedValue_min}
  Let $X$ be a discrete time supermartingale with respect to the filtration $\mathcal{F}$ taking values in a real Banach space which is also a partially ordered module and an ordered monoid. Let
  $\tau, \sigma$ be stopping times. Then, if $\tau$ is bounded, we have that almost surely,
  $X_{\tau \wedge \sigma} \ge P[X_{\tau} \mid \mathcal{F}_{\sigma}]$.
\end{lemma}
...
Thus, the result follows by Lemma~\ref{lem:conExpUI} as $\{X_n\}$ is uniformly integrable.
\end{proof}
