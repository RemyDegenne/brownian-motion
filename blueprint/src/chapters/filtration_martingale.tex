...
\begin{theorem}\label{thm:tendsto_limitProcess_of_cadlag}
  \uses{def:Martingale, def:limitProcess, def:RightContinuous, def:approximableTimeIndex,
    def:rightContinuous}
  \notready
Let $X : T \to \Omega \to \mathbb{R}$ be a right continuous martingale bounded in $L^1$ on an approximable
time index $T$ with respect to a right continuous filtration $\mathcal{F}$. Then there exists a
limit process $X_\infty$ measurable with respect to $\mathcal{F}_\infty$ such that $X_t$ converges
to $X_\infty$ almost surely as $t$ goes to infinity. Furthermore,
$X_t = P[X_\infty \mid \mathcal{F}_t]$ almost surely.
\end{theorem}
