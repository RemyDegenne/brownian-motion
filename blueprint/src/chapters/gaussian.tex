...
The multivariate Gaussian measure on $E$ with mean $\mu \in E$ and covariance operator $C : E \to E \to \mathbb{R}$ is the pushforward measure of the standard Gaussian measure on $E$ by the map $x \mapsto \mu + C^{1/2} x$, where $C^{1/2}$ is the unique positive operator such that $C^{1/2} C^{1/2} = C$.
\end{definition}
