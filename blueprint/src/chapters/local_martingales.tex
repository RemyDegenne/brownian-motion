...
We say that a process has integrable supremum if $(t, \omega) \mapsto \sup_{s \le t} \Vert X_s(\omega) \Vert$ is strongly measurable and $(\sup_{s \le t} \Vert X_s \Vert)_t$ is integrable for all $t$.
A process has locally integrable supremum if it is locally a process with integrable supremum in the sense of Definition~\ref{def:locally}.
\end{definition}
...
\begin{lemma}\label{lem:Integrable.classDL}
  \uses{def:classDL, def:locallyIntegrableSup}
  \leanok
  \lean{MeasureTheory.Integrable.classDL}
Let $X$ be a stochastic process with integrable supremum (Definition~\ref{def:locallyIntegrableSup}). Then $X$ is of class DL.
\end{lemma}
