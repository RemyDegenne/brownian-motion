...
We say that a stochastic process is jointly strongly measurable, if the associated map
$(t,\omega)\mapsto X_t(\omega)$ is strongly measurable.
\end{definition}


\begin{definition}\label{def:locallyIntegrableSup}
  \uses{def:locally, def:jointlyStronglyMeasurable}
  \leanok
...
A process has locally integrable supremum if it is jointly strongly measurable and $(\sup_{s \le t} \Vert X_s \Vert)_t$ is locally integrable.
\end{definition}
...
\end{lemma}
\begin{proof} \leanok
Let $X$ be a jointly strongly measurable process and $\tau$ be a stopping time. The stopped process
map $(\omega, t) \mapsto X_{t \wedge \tau(\omega)}(\omega)$ can be expressed as the composition $X \circ \phi$, where $\phi : \Omega \times [0, \infty) \to \Omega \times [0, \infty)$ is defined by:
$\phi(\omega, t) = (\omega, t \wedge \tau(\omega))$. Since $\tau$ is measurable, the function $\phi$ is measurable.
The composition of a strongly measurable function with a measurable function is strongly measurable.
\end{proof}
