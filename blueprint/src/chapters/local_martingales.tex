...
\begin{lemma}\label{lem:locally_induction2}
  \uses{def:locally, def:stable}
  \leanok
  \lean{ProbabilityTheory.locally_induction₂}
  Suppose that the filtration is right-continuous.
  Let \(P, Q, R\) be three stable classes of stochastic processes such that \(R \cap P \subseteq Q_{\mathrm{loc}}\). Then, \(R_{\mathrm{loc}} \cap P_{\mathrm{loc}} \subseteq Q_{\mathrm{loc}}\).
\end{lemma}

\begin{proof}\leanok
  By Lemma~\ref{lem:locally_inter}, we have \(R_{\mathrm{loc}} \cap P_{\mathrm{loc}} = (R \cap P)_{\mathrm{loc}}\).
  Thus, applying Lemma~\ref{lem:local_induction} to the inclusion \(R \cap P \subseteq Q_{\mathrm{loc}}\) concludes the proof.
\end{proof}
...
Let $t \in T$ and $\tau \le t$ be a stopping time. By Lemma~\ref{lem:optionalSamplingSubmartingale} and nonnegativity we get that $0 \le X_\tau \le P[X_t \mid X_\tau]$. Thus, we conclude by applying Lemma~\ref{lem:uniformIntegrableDominated} and Lemma~\ref{lem:condExpUI}.
\end{proof}
...
\begin{lemma}\label{lem:isStable_integrableSup}
...
\begin{proof}
Let $X$ be a process with locally integrable supremum and $\tau$ be a stopping time. Let $t \in T$. Then $(X^\tau)^*_t = \sup_{s \le t} \|X_{\tau \land s}\| \le \sup_{s \le t} \|X_s\| = X^*_t$, and as $X^*_t$ is integrable, so is $(X^\tau)^*_t$. Thus $(X^\tau \mathbb{I}_{\tau > 0})^*_t$ is integrable, concluding the proof.
\end{proof}
...
\begin{proof}
  \uses{lem:isStable_integrableSup, lem:isStable_locally}
  \leanok
  Follows by applying Lemma~\ref{lem:isStable_locally} to Lemma~\ref{lem:isStable_integrableSup}.
\end{proof}
...
\begin{proof}
  \uses{lem:Integrable.classDL, lem:locally_mono}
...
\begin{lemma}\label{lem:locally_classDL_iff_locally_classD}
  \uses{def:rightContinuous, def:locally, def:classD, def:classDL}
  \leanok
  \lean{ProbabilityTheory.locally_classDL_iff_locally_classD}
  If the filtration is right-continuous and $X$ is locally of class DL if and only if it is locally of class D.
\end{lemma}

\begin{proof}
  \uses{lem:locally_classD_of_locally_classDL, lem:classDLOfClassD, lem:locally_mono}
  \leanok
  The forward direction is Lemma~\ref{lem:locally_classD_of_locally_classDL} while the reverse
  direction follows by applying Lemma~\ref{lem:locally_mono} to Lemma~\ref{lem:classDLOfClassD}.
\end{proof}
...
Assume $T$ has a bottom element $\bot$ and that its closed intervals are compact. If $X$ is a real-valued càdlàg and adapted process, and if the filtration is right-continuous, then the sequence $\tau_n := \inf \{t \mid X_t \ge n\}$ is a localizing sequence.
\end{lemma}
...
\begin{align*}
  (X^{\tau})^*_t
  \le n + \mathbb{I}_{\tau \le t} \Vert X_{\tau} \Vert
  \: .
\end{align*}
\end{lemma}

\begin{proof}
If $\tau > t$, then for all $s \le t$, $\|X_s\| \le n$, and thus $(X^\tau)^*_t = \sup_{s \le t} \|X_{\tau \land s}\| \le n = n + \mathbb{I}_{\tau \le t} \|X_{\tau}\|$. Otherwise $(X^\tau)^*_t = \sup_{s \le \tau} \|X_s\|$. For $s < \tau$, $\|X_s\| \le n$, and $\|X_\tau\| \le \|X_\tau\|$ so $\sup_{s \le \tau} \|X_s\| \le n \lor \|X_\tau\| \le n + \|X_\tau\| = n + \mathbb{I}_{\tau \le t} \|X_\tau\|$.
\end{proof}
...
Assume $T$ has a bottom element $\bot$ and that its closed intervals are compact. If $X$ is an a.s. càdlàg and adapted process, and if the filtration is right-continuous, then any process locally of class DL has locally integrable supremum.
\end{lemma}

\begin{proof}
  \uses{lem:local_induction, lem:ClassDL.hasLocallyIntegrableSup, lem:isStable_hasLocallyIntegrableSup}
  \leanok
Apply Lemma~\ref{lem:local_induction2} using Lemma~\ref{lem:ClassDL.hasLocallyIntegrableSup}, Lemma~\ref{lem:isStable_hasLocallyIntegrableSup} and Lemma~\ref{lem:locally_isCadlag}.
\end{proof}
...
\begin{align*}
  (X^{\tau_n})^*_t
  & \le n + \mathbb{I}_{\tau_n \le t} \Vert X_{\tau_n} \Vert \\
