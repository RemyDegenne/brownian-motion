...
\begin{proof}\leanok
  \uses{lem:isStronglyPredictable.predictablePart_eq}
...
\begin{theorem}[Doob-Meyer decomposition]\label{thm:Doob_Meyer}
  \uses{def:Submartingale, def:classD, def:predictable, def:Martingale}
...
\begin{lemma}\label{lem:submartingale_iff_integrable}
  \uses{def:Submartingale, def:HasIntegrableSup}
An adapted increasing process $A$ is a submartingale iff it is integrable.
\end{lemma}
...
\begin{lemma}\label{lem:local_doobMeyer_increasing}
  \uses{def:IsLocalMartingale, def:predictable}
An adapted locally integrable increasing process $A$ can be written as a sum of a local martingale and a predictable locally integrable increasing process.
\end{lemma}

\begin{proof}
  \uses{thm:local_doobMeyer, lem:submartingale_iff_integrable}
A locally integrable increasing process is a local submartingale, thus we can apply Theorem~\ref{thm:local_doobMeyer}.
\end{proof}


See Lemma~\ref{lem:local_doobMeyer_integrable_variation} further down for an application to processes with locally integrable variation.
