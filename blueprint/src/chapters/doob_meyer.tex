...
\begin{lemma}\label{lem:predictablePart_succ}
  \uses{def:predictablePart}
  \leanok
  \lean{MeasureTheory.predictablePart_succ}
...
\begin{lemma}\label{lem:adapted_predictablePart}
  \uses{def:predictablePart}
  \mathlibok
  \lean{MeasureTheory.adapted_predictablePart}
...
\begin{proof}\leanok
  \uses{def:Submartingale, lem:condExp_sub_nonneg}
Let $X$ be a submartingale and let $A$ be its predictable part. Then for all $n \geq 0$, almost surely,
\begin{align*}
  A_{n+1} &= A_n + \mathbb{E}\left[ X_{n+1} - X_n | \mathcal{F}_n \right] \\
  &\ge A_n
  \: .
\end{align*}
The last inequality follows from Lemma~\ref{lem:condExp_sub_nonneg}. As $\mathbb{N}$ is countable, we deduce that almost surely, for all $n \in \mathbb{N}$, $A_{n+1} \ge A_n$.
