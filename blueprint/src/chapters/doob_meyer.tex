...
\begin{lemma}\label{lem:komlos_convex}
  \leanok
  \lean{komlos_convex}
Let $(f_n)_{n\in\mathbb{N}}$ be a sequence in a vector space $E$ and $\phi : E \to \mathbb{R}_+$ be a convex function such that $\phi(f_n)$ is a bounded sequence.
...
\begin{lemma}\label{lem:komlos_nonneg_L1}
Let $(f_n)_{n\in\mathbb{N}}$ be a sequence of random variables with values in $[0, \infty]$.
Then there exist functions $g_n \in convex( f_n, f_{n+1}, \cdots)$ such that $(g_n)_{n\in\mathbb{N}}$ converges almost surely to a random variable $g$.
\end{lemma}
