...
$\mathcal{M}^2(E)$ is a real vector space.
\end{lemma}
...
\begin{lemma}\label{lem:IsSquareIntegrable.eLpNorm_two_mono}
  \uses{def:IsSquareIntegrable}
For $M \in \mathcal{M}^2(E)$, the function $t \mapsto \Vert M_t \Vert_{L^2}$ is non-decreasing.
\end{lemma}
...
For $M \in \mathcal{M}^2(E)$, we have $M_t \to M_\infty$ almost surely and in $L^2$ as $t \to \infty$~.
\end{lemma}
...
\begin{lemma}\label{lem:IsSquareIntegrable.sup_eLpNorm_eq_eLpNorm_limitProcess}
  \uses{def:IsSquareIntegrable, def:limitProcess}
For $M \in \mathcal{M}^2(E)$, we have
\begin{align*}
...
\begin{proof}
  \uses{lem:IsSquareIntegrable.eLpNorm_two_mono}

\end{proof}


\begin{definition}\label{def:IsSquareIntegrable.norm}
  \uses{def:IsSquareIntegrable, def:limitProcess}
...
\begin{lemma}\label{lem:IsSquareIntegrable.norm_eq_zero}
  \uses{def:IsSquareIntegrable.norm}
For $M \in \mathcal{M}^2(E)$, $\Vert M \Vert = 0$ if and only if $M$ is the zero process.
\end{lemma}
...
\begin{definition}\label{def:IsSquareIntegrable.inner}
  \uses{def:IsSquareIntegrable.norm}
...
\begin{theorem}\label{thm:hilbertSpace_isSquareIntegrable}
  \uses{def:IsSquareIntegrable.inner, lem:IsSquareIntegrable.norm_eq_zero}
The space $\mathcal{M}^2$ is a Hilbert space.
\end{theorem}

\begin{proof}
  \uses{cor:doob_lp_norm, lem:IsSquareIntegrable.module}
