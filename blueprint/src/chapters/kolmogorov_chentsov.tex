...
Let the \emph{log-size radius} of $(V,t)$, denoted by $r_{V,t}$, be the smallest positive integer $r$ such that $\vert B_V(t, r c) \vert \le b a^{r}$.
\end{definition}


\begin{lemma}\label{lem:card_logSizeRadius_ge}
  \uses{def:logSizeRadius}
For $r$ the log-size radius of $(V,t)$, we have $b a^{r-1} \le \vert B_V(t, (r-1)c) \vert$~.
\end{lemma}
...
\begin{lemma}\label{lem:card_logSizeRadius_le}
  \uses{def:logSizeRadius}
For $r$ the log-size radius of $(V,t)$, we have $\vert B_V(t, rc) \vert \le b a^r$~.
\end{lemma}
...
\begin{itemize}
  ...
  \item for all $i$, $r_i$ is the log-size radius of $(V_i, t_i)$,
...
We define $K = \bigcup_{i=0}^{m-1} K_i$ for $m$ as in Lemma~\ref{lem:logSizeBallSequence_eq_zero}, set of pairs from the log-size ball sequence.
\end{definition}
...
\begin{lemma}\label{lem:sup_dist_le_two_mul_sup_dist_pairSet}
  \uses{def:pairSet}
Let $K$ be the pair set of a log-size ball sequence for $(J, a, b, c, n)$.
...
Let $p, q > 0$.
We say that $X$ satisfies the Kolmogorov condition for $(p,q)$ with constant $M$ if for all $s, t \in T$,
\begin{align*}
...
Let $X : T \to \Omega \to E$ be a process that satisfies the Kolmogorov condition for $(p,q)$ with constant $M$.
...
Let $X : T \to \Omega \to E$ be a process that satisfies the Kolmogorov condition for $(p,q)$ with constant $M$, with $q > d$ and $p > 0$.
For all $\delta > 0$,
\begin{align*}
  &\mathbb{E}\left[ \sup_{s, t \in T; d_T(s, t) \le \delta} d_E(X_s, X_t)^p \right]
  \\
