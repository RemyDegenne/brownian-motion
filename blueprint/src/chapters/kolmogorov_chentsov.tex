...
\begin{proof}
  \uses{cor:finite_set_bound_of_dist_le_of_diam_le, cor:finite_set_bound_of_dist_le_of_le_diam, lem:integral_div_dist_le_sum_integral_dist_le}
...
\begin{align*}
  ...
  \: .
\end{align*}
Let $a_k = 2^{k \beta p} 4^{p+2q+1} M c_1 (\mathrm{diam}(T)+1)^{q-d} 2^{(-k + 1)(q-d)} \left(4^d \left(\max\left\{0, \log_2(c_1) + (k + 1)d \right\}\right)^q
    + \frac{1}{\left( 2^{(q -d)/p} - 1\right)^p}\right)$.
