...
\begin{align*}
  ...
  &\le 2 \left(\sum_{i=m}^{k-1} \left( \mathbb{E}\left[\sup_{t \in C_k} d_E(X_{\bar{t}_i}, X_{\bar{t}_{i+1}})^p\right] \right)^{1/p}\right)^p
  \: .
\end{align*}
\end{lemma}
...
\begin{align*}
  \left(\mathbb{E} \left[\sup_{t \in C_k} d_E(X_t, X_{\bar{t}_m})^p \right]\right)^{1/p}
  &\le \max\{1, 2^{1/p - 1}\}\sum_{i=m}^{k-1} \left( \mathbb{E}\left[\sup_{t \in C_k} d_E(X_{\bar{t}_i}, X_{\bar{t}_{i+1}})^p \right] \right)^{1/p}
  \: .
\end{align*}
The $\max\{1, 2^{1/p - 1}\}$ factor comes from the two versions of Minkowski's inequality depending on whether $p \ge 1$ or $0 < p \le 1$.
Finally, we raise to the $p$-th power to obtain
\begin{align*}
  \mathbb{E} \left[\sup_{t \in C_k} d_E(X_t, X_{\bar{t}_m})^p \right]
  &\le \max\{1, 2^{1 - p}\}\left(\sum_{i=m}^{k-1} \left( \mathbb{E}\left[\sup_{t \in C_k} d_E(X_{\bar{t}_i}, X_{\bar{t}_{i+1}})^p \right] \right)^{1/p} \right)^p
  \\
  &\le 2 \left(\sum_{i=m}^{k-1} \left( \mathbb{E}\left[\sup_{t \in C_k} d_E(X_{\bar{t}_i}, X_{\bar{t}_{i+1}})^p \right] \right)^{1/p} \right)^p
  \: .
\end{align*}
...
\begin{proof}
  \uses{thm:finite_set_bound_of_dist_le, lem:internalCoveringNumber_eq_one_of_diam_le}
...
\begin{corollary}\label{cor:finite_set_bound_of_dist_le_bis}
  \uses{def:IsKolmogorovProcess, def:HasBoundedInternalCoveringNumber}
  \leanok
  \lean{ProbabilityTheory.finite_set_bound_of_edist_le_bis}
...
\begin{proof}
  \uses{cor:finite_set_bound_of_dist_le_of_le_diam, cor:finite_set_bound_of_dist_le_of_diam_le}
...
\begin{proof}
  \uses{cor:finite_set_bound_of_dist_le_bis, lem:integral_div_dist_le_sum_integral_dist_le, lem:L_lt_top}
