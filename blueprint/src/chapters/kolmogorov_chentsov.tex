...
\section{Chaining for stochastic processes satisfying the Kolmogorov condition}
...
Let $p, q > 0$.
We say that $X$ satisfies the Kolmogorov condition for exponents $(p,q)$ with constant $M$ if for all $s, t \in T$,
\begin{align*}
  \mathbb{E}[d_E(X_s, X_t)^p] \le M d_T(s, t)^q
  \: ,
\end{align*}
and if furthermore $(X_s, X_t)$ is measurable for all $s, t \in T$.
\end{definition}
...
%   \uses{def:HasBoundedInternalCoveringNumber, def:IsKolmogorovProcess}
%   \notready
...
% Assume that $X = (X_t)_{t\in I}$ is an $E$-valued stochastic process and there are $\alpha, \beta, c_2>0$ such that $X$ satisfies the Kolmogorov condition for $(\alpha, d+\beta)$ with constant $c_2$.
...
% \begin{proof}\uses{thm:countable_set_bound}
...
\begin{definition}\label{def:HasSigmaBoundedCoveringNumber}
  \uses{def:HasBoundedInternalCoveringNumber}
A set $T$ has sigma-bounded covering number if there exists a monotone sequence of totally bounded open subsets $(T_n)_{n \in \mathbb{N}}$ of $T$ such that for all $n$, $T_n$ has bounded internal covering number with constant $c_n>0$ and exponent $d_n > 0$, and such that $\bigcup_{n \in \mathbb{N}} T_n = T$.
\end{definition}


\begin{definition}\label{def:IsLocalizedKolmogorovProcess}
  \uses{def:IsKolmogorovProcess, def:HasSigmaBoundedCoveringNumber}
Let $T$ be a metric space with sigma-bounded covering number.
A stochastic process $X : T \to \Omega \to E$ satisfies the localized Kolmogorov conditions if $(X_s, X_t)$ is measurable for all $s, t \in T$ and if for all $n \in \mathbb{N}$, there are exponents $(p_n, q_n)$ with $p_n > 0$ and $q_n > d_n$ and a constant $M_n > 0$ as well as $\rho_n > 0$ such that for all $s, t \in T_n$ with $d_T(s, t) \le \rho_n$,
\begin{align*}
