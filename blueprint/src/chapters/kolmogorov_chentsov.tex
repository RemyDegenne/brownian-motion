...
\begin{proof}
  \uses{lem:chain, lem:integral_sup_rpow_dist_succ_pairs}
...
\begin{align*}
  \mu\left[ \sup_{s, t \in T; d_T(s, t) \le \delta} d_E(X_s, X_t)^p \right] \: .
\end{align*}
Note the condition that the supremum is taken over pairs $(s, t)$ such that $d_T(s, t) \le \delta$.

TODO: we change scale.
From a set of points in a cover $C_k$ which are not far apart, we build a set of points in $C_m$ for $m \le k$ which are not far apart.
...
\begin{align*}
  \mu\left[ \sup_{s, t \in J;\: s \ne t} \frac{d_E(X_s, X_t)^p}{d_T(s, t)^{\beta p}} \right]
  &\le \sum_{k=0}^\infty \mu\left[ \sup_{s, t \in J;\: s \ne t, \: \eta_k < d_T(s, t) \le 2 \eta_k} \frac{d_E(X_s, X_t)^p}{d_T(s, t)^{\beta p}} \right]
  \\
  &\le \sum_{k=0}^\infty \eta_k^{-\beta p} \mu\left[ \sup_{s, t \in J;\: s \ne t, \: d_T(s, t) \le 2 \eta_k} d_E(X_s, X_t)^p \right]
  \\
  &\le \sum_{k=0}^\infty 2^{k \beta p} \mu\left[ \sup_{s, t \in J;\: s \ne t, \: d_T(s, t) \le 2 \eta_k} d_E(X_s, X_t)^p \right]
  \: .
\end{align*}
...
\begin{align*}
  &\mu\left[ \sup_{s, t \in T; d_T(s, t) \le 2 \eta_k} d_E(X_s, X_t)^p \right]
  \\
...
\begin{align*}
  \sum_{k=1}^{k_0 - 1} 2^{k \beta p} \mu\left[ \sup_{s, t \in J;\: s \ne t, \: d_T(s, t) \le 2 \eta_k} d_E(X_s, X_t)^p \right]
...
\begin{theorem}\label{thm:holder_version}
  \uses{def:IsKolmogorovProcess, def:HasBoundedInternalCoveringNumber}
Under the assumptions of Theorem~\ref{thm:countable_set_bound}, for $E$ a complete space, there exists a version $Y$ of $X$ (i.e., a process $Y$ with $\mu(Y_t \ne X_t) = 0$ for all $t$) such that all the paths of $Y$ are Hölder continuous of all orders $\gamma \in (0, (q - d)/p)$.
\end{theorem}
