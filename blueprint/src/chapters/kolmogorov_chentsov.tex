...
\begin{proof}
  \uses{len:dist_chainingSequence_le_sum}
...
\begin{align*}
  \mathbb{E}\left[ \sup_{s, t \in T; d_T(s, t) \le \delta} d_E(X_s, X_t)^p \right]
  &\le 4^{p+2q+1} M \delta^{q-d} \frac{c_1}{\left( 2^{(q -d)/p} - 1\right)^p}
  \: .
\end{align*}
\end{corollary}
...
\begin{proof}
  \uses{thm:finite_set_bound_of_dist_le, cor:finite_set_bound_of_dist_le_of_le_diam, lem:integral_div_dist_le_sum_integral_dist_le, lem:internalCoveringNumber_eq_one_of_diam_le}
...
For $k < k_0$, $N^{int}_{\eta_{k+1}}(T) = 1$ (Lemma~\ref{lem:internalCoveringNumber_eq_one_of_diam_le}). Applying Theorem~\ref{thm:finite_set_bound_of_dist_le} with $\delta = 2 \eta_k$, we obtain
\begin{align*}
