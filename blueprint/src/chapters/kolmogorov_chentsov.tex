...
Let $X : T \to \Omega \to E$ be process that satisfies the Kolmogorov condition for $(p,q)$ with constant $M$.
...
\begin{theorem}\label{thm:finite_set_bound_of_dist_le}
  \uses{def:IsKolmogorovProcess, def:chainingSequence, def:HasBoundedInternalCoveringNumber}
Suppose that $T$ is a finite set with bounded internal covering number with constant $c_1>0$ and exponent $d > 0$.
Let $X : T \to \Omega \to E$ be process that satisfies the Kolmogorov condition for $(p,q)$ with constant $M$, with $q > d$ and $p > 0$.
For all $\delta > 0$,
\begin{align*}
  &\mu\left[ \sup_{s, t \in T; d_T(s, t) \le \delta} d_E(X_s, X_t)^p \right]
  \\
...
\begin{align*}
  \mu \left[ \sup_{s, t \in C_k; d_T(s, t) \le \delta} d_E(X_{\bar{s}_m}, X_{\bar{t}_m})^p \right]
  &\le 2^{p+2} M \left(\frac{16}{\log 2} \delta \log N^{int}_{\delta/4}(T) \right)^q  N^{int}_{\delta/4}(J)
  \: . \\
  \mu \left[\sup_{t \in C_k} d_E(X_t, X_{\bar{t}_m})^p \right]
...
\begin{align*}
  &\mu\left[ \sup_{s, t \in C_k; d_T(s, t) \le \delta} d_E(X_s, X_t)^p \right]
  \\
  &\le 4^p M \left(4\left(\frac{16}{\log 2} \delta \log N^{int}_{\delta/4}(T) \right)^q  N^{int}_{\delta/4}(T)
...
Let $X : T \to \Omega \to E$ be process that satisfies the Kolmogorov condition for $(p,q)$ with constant $M$, with $q > d$ and $p > 0$.
Let $\beta \in(0, (q - d)/p)$.
Then there exists a constant $L(T, c_1, d, p, q, \beta) < \infty$ such that
\begin{align*}
...
Build a monotone sequence of finite sets $T_n \subseteq T'$, use Lemma~\ref{lem:finite_set_bound} to obtain a bound for each $T_n$, and then use monotone convergence.
\end{proof}


\begin{corollary}\label{cor:holder_version}
  \uses{def:IsKolmogorovProcess, def:HasBoundedInternalCoveringNumber}
