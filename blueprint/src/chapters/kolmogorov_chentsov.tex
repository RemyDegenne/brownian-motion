...
It remains to show that $Y$ is a version of $X$.
...
\begin{proof}
  \uses{lem:holder_version_single, lem:indistinguishable_of_version_of_continuous}
Let $(\beta_n)$ be an increasing sequence of numbers in $(0, (q - d)/p)$ such that $\beta_n \to (q - d)/p$.
For each $n$, let $Y^n$ be the version of $X$ given by Lemma~\ref{lem:holder_version_single} for $\beta = \beta_n$.
Then by Lemma~\ref{lem:indistinguishable_of_version_of_continuous}, the processes $Y^0$ and $Y^n$ are indistinguishable for all $n$.
