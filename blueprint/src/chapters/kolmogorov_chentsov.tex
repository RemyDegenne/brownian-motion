...
For $k < k_0$, we apply Corollary~\ref{cor:finite_set_bound_of_dist_le_of_diam_le} with $\delta = 2 \eta_k$, we obtain
\begin{align*}
  &\mathbb{E}\left[ \sup_{s, t \in T; d_T(s, t) \le 2 \eta_k} d_E(X_s, X_t)^p \right]
  \\
  &\le 4^{p+2q+1} M c_1 (2 \eta_k)^{q-d} \frac{1}{\left( 2^{(q -d)/p} - 1\right)^p}
  \\
