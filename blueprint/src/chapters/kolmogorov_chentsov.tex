...
We follow the presentation of the Kolmogorov-Chentsov theorem in \cite{kratschmer2023kolmogorov}, which builds upon the chaining technique developed by Talagrand \cite{talagrand2022upper}.
...
Let $V$ be a finite subset of a metric space and let $t \in V$ and $a > 1$, $b, c > 0$.
Let the \emph{log-size radius} of $t$ in $V$, denoted by $r_{V,t}$, be the smallest positive integer $r$ such that $\vert B_V(t, r c) \vert \le b a^{r}$.
\end{definition}


\begin{lemma}\label{lem:card_logSizeRadius_ge}
  \uses{def:logSizeRadius}
$b a^{r_{V,t}-1} \le \vert B_V(t, (r_{V,t}-1)c) \vert$~.
\end{lemma}
...
\begin{lemma}\label{lem:card_logSizeRadius_le}
  \uses{def:logSizeRadius}
$\vert B_V(t, r_{V,t}c) \vert \le b a^{r_{V,t}}$~.
\end{lemma}
...
An log-size ball sequence for $(J, a, b, c, n)$ is a sequence of $(V_i, t_i, r_i)_{i \in \mathbb{N}}$ such that
\begin{itemize}
...
The radius of a log-size ball sequence $(V_i, t_i, r_i)_{i \in \mathbb{N}}$ for $(J, a, b, c, n)$ satisfies $r_i \le n$ for all $i \in \mathbb{N}$.
\end{lemma}

\begin{proof}
Since $|J| \le b a^n$, we have $\vert B_{V_i}(t_i, n c) \vert \le \vert J \vert \le b a^{n}$.
\end{proof}
...
\begin{lemma}\label{lem:logSizeBallSequence_eq_zero}
  \uses{def:logSizeBallSequence}
For any log-size ball sequence $(V_i, t_i, r_i)_{i \in \mathbb{N}}$ for $(J, a, b, c, n)$, there exists $m \in \mathbb{N}$ such that $V_k = \emptyset$ for all $k \ge m$.
\end{lemma}

\begin{proof}
  \uses{lem:logSizeBallSequence_V_anti}
$V_{i+1} = V_i \setminus B_{V_i}(t_i, (r_i - 1)c)$ and since $t_i \in B_{V_i}(t_i, (r_i - 1)c)$, we have $\vert V_{i+1} \vert < \vert V_i \vert$ and the cardinal eventually reaches $0$.
\end{proof}
...
\begin{definition}\label{def:pairSet}
  \uses{def:logSizeBallSequence, lem:logSizeBallSequence_eq_zero}
Let $(V_i, t_i, r_i)_{i \in \mathbb{N}}$ be a log-size ball sequence for $(J, a, b, c, n)$.
For $i \in \mathbb{N}$, let $K_i = \{t_i\} \times B_{V_i}(t_i, r_i c)$ be the set of pairs $(t_i, s)$ for $s$ in the ball $B_{V_i}(t_i, r_i c)$.
We define $K = \bigcup_{i=0}^{m-1} K_i$ for $m$ as in Lemma~\ref{lem:logSizeBallSequence_eq_zero}, set of all pairs from the log-size ball sequence.
\end{definition}


\begin{lemma}\label{lem:card_pairSet_le}
  \uses{def:pairSet}
The cardinal of the pair set $K$ of a log-size ball sequence for $(J, a, b, c, n)$ satisfies $|K| \le a |J|$.
\end{lemma}
...
\begin{lemma}\label{lem:dist_le_of_mem_pairSet}
  \uses{def:pairSet}
Let $(s, t)$ be a pair in the pair set $K$ of a log-size ball sequence for $(J, a, b, c, n)$.
Then $d_T(s, t) \le c n$.
\end{lemma}
...
\begin{lemma}\label{lem:sup_dist_le_two_mul_sup_dist_pairSet}
  \uses{def:pairSet}
Let $K$ be the pair set of a log-size ball sequence $(V_i, t_i, r_i)_{i \in \mathbb{N}}$ for $(J, a, b, c, n)$.
...
Let $(V_i, t_i, r_i)_{i \in \mathbb{N}}$ be a log-size ball sequence for $(J, a, b, c, n)$. We show that its pair set satisfies the conditions of the lemma.
...
Remark: the measurability condition on the pair could be replaced by the simpler condition of measurability of $X_t$ for all $t \in T$ if we assumed that $E$ is separable (which implies that the Borel $\sigma$-algebra on the product is equal to the product of the Borel $\sigma$-algebras).
...
\begin{lemma}\label{lem:integral_sup_rpow_dist_succ_pairs}
  \uses{def:IsKolmogorovProcess}
...
\begin{proof}
  \uses{lem:pair_reduction, lem:integral_sup_rpow_dist_succ_pairs}
...
\begin{align*}
  ...
  \: .
\end{align*}
Then by Lemma~\ref{lem:integral_sup_rpow_dist_succ_pairs},
\begin{align*}
...
\begin{proof}
  \uses{lem:dist_chainingSequence_add_one, lem:integral_sup_rpow_dist_succ_pairs}
\begin{align*}
  \mathbb{E}\left[\sup_{t \in C_k} d_E(X_{\bar{t}_j}, X_{\bar{t}_{j+1}})^p \right]
  &\le \mathbb{E}\left[\sum_{u \in C_{j+1}} d_E(X_{\bar{u}_j}, X_{u})^p \right]
  \: .
\end{align*}
We then apply Lemma~\ref{lem:integral_sup_rpow_dist_succ_pairs} to the set $C = \{(\bar{u}_j, u) \mid u \in C_{j+1}\}$, which satisfies the condition $d_T(\bar{u}_j, u) \le \varepsilon_j$ and has cardinal $\vert C_{j+1} \vert$.
\end{proof}
...
\begin{align*}
  ...
  \\
  &\le 2^d M c_1 (\varepsilon_0 2^{-m})^{q - d} \frac{1}{\left( 1 - 2^{- (q -d)/p}\right)^p}
  \: .
\end{align*}
\end{corollary}
...
\begin{align*}
  ...
  \: . \\
  \mathbb{E} \left[\sup_{t \in C_k} d_E(X_t, X_{\bar{t}_m})^p \right]
  &\le2^d M c_1 (\varepsilon_0 2^{-m})^{q - d} \frac{1}{\left( 1 - 2^{- (q -d)/p}\right)^p}
  \: .
\end{align*}
...
\begin{align*}
  ...
  &\le 4^p M \left(4\left(16 \delta \log_2 N^{int}_{\delta/4}(T) \right)^q  N^{int}_{\delta/4}(T)
    + 2^d c_1 (\varepsilon_0 2^{-m})^{q - d} \frac{1}{\left( 1 - 2^{- (q -d)/p}\right)^p}\right)
  \: .
\end{align*}
...
\begin{align*}
  ...
  &\le 4^p M \left(4^{2q+1}\delta^q \left(\log_2 N^{int}_{\delta/4}(T) \right)^q  N^{int}_{\delta/4}(T)
    + c_1 2^d \delta^{q - d} \frac{1}{\left( 1 - 2^{- (q -d)/p}\right)^p}\right)
  \\
  &=   4^p M \left(4^{2q+1}\delta^q \left(\log_2 N^{int}_{\delta/4}(T) \right)^q  N^{int}_{\delta/4}(T)
...
\begin{proof}
  \uses{thm:finite_set_bound_of_dist_le, cor:finite_set_bound_of_dist_le_of_le_diam}
Let $\eta_k = 2^{-k}(\mathrm{diam}(T) + 1)$ for $k \in \mathbb{N}$.
We will often use the simple fact that $\eta_k \ge 2^{-k}$.
We will bound separately for each $k \in \mathbb{N}$ the expectation of the supremum over the sets of pairs $(s, t)$ such that $\eta_k < d_T(s, t) \le 2 \eta_k$.
\begin{align*}
  \mathbb{E}\left[ \sup_{s, t \in J;\: s \ne t} \frac{d_E(X_s, X_t)^p}{d_T(s, t)^{\beta p}} \right]
...
\begin{lemma}\label{lem:holder_version_single}
  \uses{def:IsKolmogorovProcess, def:HasBoundedInternalCoveringNumber}
Under the assumptions of Theorem~\ref{thm:countable_set_bound}, for $E$ a complete space and $\beta \in (0, (q - d)/p)$, there exists a version $Y$ of $X$ (i.e., a process $Y$ with $\mathbb{P}(Y_t \ne X_t) = 0$ for all $t$) such that almost surely the paths of $Y$ are Hölder continuous of order $\beta$.
\end{lemma}
...
Let $x_0 \in E$ be arbitrary and let $\overline{X} : T \to \Omega \to E$ be the process defined by
\begin{align*}
  \overline{X}_t(\omega)
  &= \begin{cases}
  ...
  \: .
\end{align*}
Then $\overline{X}$ has Hölder continuous paths of order $\beta$ almost surely.

We can show that $(\overline{X}_s, \overline{X}_t)$ is $\mathbb{P}$-a.e. measurable for all $s, t \in T$.

It remains to show that $\overline{X}$ is a version of $X$.

TODO
\end{proof}


\begin{theorem}\label{thm:holder_version}
  \uses{def:IsKolmogorovProcess, def:HasBoundedInternalCoveringNumber}
Under the assumptions of Theorem~\ref{thm:countable_set_bound}, for $E$ a complete space, there exists a version $Y$ of $X$ (i.e., a process $Y$ with $\mathbb{P}(Y_t \ne X_t) = 0$ for all $t$) such that almost surely the paths of $Y$ are Hölder continuous of all orders $\gamma \in (0, (q - d)/p)$.
\end{theorem}

\begin{proof}
  \uses{lem:holder_version_single}

\end{proof}



\subsection{Localized version of the Kolmogorov-Chentsov theorem}


\begin{definition}[Cover with bounded covering numbers]\label{def:HasSigmaBoundedCoveringNumber}
  \uses{def:HasBoundedInternalCoveringNumber}
...
\begin{lemma}\label{lem:hasSigmaBoundedCoveringNumber_nnreal}
  \uses{def:HasSigmaBoundedCoveringNumber}
...
\begin{definition}[Localized Kolmogorov conditions]\label{def:IsLocalizedKolmogorovProcess}
  \uses{def:IsKolmogorovProcess, def:HasSigmaBoundedCoveringNumber}
...
\begin{theorem}\label{thm:localized_holder_version}
  \uses{def:IsLocalizedKolmogorovProcess, def:HasSigmaBoundedCoveringNumber}
Let $T$ be a metric space with a cover $(T_n)$ with bounded covering numbers with constants $c_n$ and exponents $d_n$.
Let $X : T \to \Omega \to E$ be a process that satisfies the localized Kolmogorov conditions with exponents $(p_n, q_n)$ with $q_n > d_n$.
Then $X$ has a version $Y$ such that almost surely the paths of $Y$ are Hölder continuous of all orders $\gamma \in \bigcap_n [0, (q_n - d_n)/p_n)$, in which ``Hölder continuous of order 0'' means ``uniformly continuous''.
