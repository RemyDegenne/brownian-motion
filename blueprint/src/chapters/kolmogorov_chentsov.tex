...
\begin{proof}
  \uses{lem:second_term_bound, lem:integral_sup_rpow_dist_cover_rescale, cor:scale_change_rpow}
...
\begin{align*}
  ...
  \: .
\end{align*}
By Lemma~\ref{lem:integral_sup_rpow_dist_cover_rescale} and Lemma~\ref{lem:second_term_bound},
\begin{align*}
  \mathbb{E} \left[ \sup_{s, t \in C_k; d_T(s, t) \le \delta} d_E(X_{\bar{s}_m}, X_{\bar{t}_m})^p \right]
...
\begin{align*}
  ...
  &\le 4^p M \left(4\left(16 \delta \log_2 N^{int}_{\delta/4}(T) \right)^q  N^{int}_{\delta/4}(T)
    + 2^d c_1 (\varepsilon_0 2^{-m+1})^{q - d} C_p\right)
  \: .
\end{align*}
...
\begin{align*}
  \mathbb{E}\left[ \sup_{s, t \in T; d_T(s, t) \le \delta} d_E(X_s, X_t)^p \right]
  &\le 4^{p+2q+1} M c_1 \delta^{q-d} C_p
  \: .
\end{align*}
\end{corollary}
