...
\todor{The Krätschmer paper also does not mention separability of $T$ for the second part of the theorem, but they take a countable dense subset of $T$ in the proof, which requires $T$ to be second-countable.}
...
\todor{say somewhere that this is a recent and general version of the theorem, and that simpler versions exist in simpler cases, e.g. $T = [0,1]$ and $E = \mathbb{R}^n$.
Talk briefly about the dyadics proof vs chaining.}

\subsection{The Kolmogorov condition and almost everywhere measurability}

\begin{definition}[Kolmogorov condition]
...
If $E$ was second-countable\todorm{write second-countable or separable?}, then we could simply assume that $X_s$ is measurable for all $s \in T$, and the measurability of the pair would follow.

\todor{talk about the almost everywhere version of this, and almost everywhere versions of processes in general}
