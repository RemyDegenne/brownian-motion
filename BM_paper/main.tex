...
TODO: how do we justify it without repeating the discussion in the proof below?
...
In particular, we explain why the pair reduction lemma (TODO REF\todorm{}) is needed in the proof.
...
Note that this needs $\delta \le 4 \mathrm{diam}(T)$, and we omit here the case $\delta > 4 \mathrm{diam}(T)$ for which we change the scale to the singleton cover $C_0$ and the proof is a simpler version of what follows (\docs{ProbabilityTheory.finite_set_bound_of_edist_le_of_diam_le}).
\begin{align*}
...
We bound the supremum by a sum and apply the inequality~\eqref{eq:kolmogorov_condition_finite_set} for $K = \{(s,t) \in C_m^2 \mid d_T(s, t) \le 8 \varepsilon_m\}$ with cardinal $\lvert K \rvert \le \lvert C_m \rvert^2$, and then the covering number assumption:
\begin{align*}
...
The issue is the square on the cardinal of the cover $C_m$, and this is what is fixed by the pair reduction lemma (Lemma~\ref{lem:pair_reduction}).
