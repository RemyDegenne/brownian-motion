...
All references to results in \mathlib are accurate for commit {\tt xxx}, August 22, 2025\todorm{move this?}.
...
We begin by defining set systems we will need, which correspond to the \mathlib definitions \docs{MeasureTheory.IsSetSemiring} and \docs{MeasureTheory.IsSetRing}.

\begin{definition}[Semi-ring, ring]\label{def:semi}
  Let $E$ be some set. We call $\mathcal H \subseteq 2^E$ a
...
In order to apply Theorem~\ref{T:masseind}\todorm{to what? I'm lost}, we need to show that the additive content is $\sigma$-additive. We do this on the level of rings rather than semi-rings. Note that we can extend an additive content on a semi-ring to an additive content on the ring generated by the semi-ring.
...
\begin{definition}[Inner regularity \docs{MeasureTheory.Measure.InnerRegularWRT}] \label{def:innerreg}
  ...
  \item If $q(A) = \text{true}$ iff $A$ is measurable, we neglect the
    {\em and $q$}\todorm{what does that mean?}. If $p(A) = \text{true}$ iff $A$ is closed (compact,
...
We also require the underlying space(s) to be second-countable. A second-countable uniform space can be made into an (extended) pseudo-metric space (\docs{UniformSpace.pseudoMetrizableSpace}).
...
\todor{Everywhere in the paper: be clear about what we formalized and what was already in mathlib.}
...
\begin{definition}
  The \emph{covariance bilinear form} (\docs{ProbabilityTheory.covarianceBilin}) of a measure $\mu$ with finite second moment $\mathbf{E}_\mu[\Vert X \Vert^2]$ is the bilinear form $C_{\mu} : E \times E \to \R$ defined by
  $$C_\mu(s, t) = \mathrm{cov}_\mu(\langle s, \cdot \rangle, \langle t, \cdot \rangle).$$
\end{definition}

The properties of the covariance allow to easily deduce that $C_\mu$ is actually a continuous and positive semidefinite bilinear form. We have the following characterization of Gaussian measures (\docs{ProbabilityTheory.isGaussian_iff_charFun_eq}).
...
While this is relatively easy to prove in finite dimension and is all we would need to define a Brownian motion on $\mathbb{R}_+$, we proved a more general result, applicable to Banach spaces, called Fernique's theorem \cite{fernique1970integrabilite} (\docs{ProbabilityTheory.IsGaussian.exists_integrable_exp_sq}).
...
\begin{lemma}[\cite{kratschmer2023kolmogorov} Lemma 6.1, extension of \cite{talagrand2014}, Lemma B.2.7]\label{lem:pair_reduction}
...
This section does not cover all details of the proof, for which we refer to \cite{kratschmer2023kolmogorov} and to the blueprint\todorm{link}. We only highlight the main steps, key arguments, and places that are relevant to the formalization specifically.
...
\begin{align*}
  \mathbb{E}\left[\sup_{s, t \in C_N, d_T(s, t) \le \delta} d_E(X_s, X_t)^p\right]
  &\le 2^p \mathbb{E}\left[\sup_{s', t' \in C_m, d_T(s', t') \le 8 \varepsilon_m} d_E(X_{s'}, X_{t'})^p\right] + 4^p \mathbb{E}\left[\sup_{s \in C_N} d_E(X_s, X_{\bar{s}_m})^p\right]
  \: .
\end{align*}
...
\todor{explain the last part of the proof about being a modification? refer to the paper? It caused us to refactor convergence in probability in Mathlib to allow extended metric spaces.}
