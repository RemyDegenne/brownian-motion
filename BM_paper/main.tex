...
In order to apply Theorem~\ref{T:masseind} and show the extension theorem, we only need to show that $\{\pi_J^{-1} C: C \in E_J \text{ compact and closed}\}$ is a compact system. Note that compact sets are closed in Hausdorff spaces, but we do not have this property since we are working with pseudo-metric spaces, which do not have this property.\footnote{Recall, that a pseudo-metric $r(.,.)$ can have $r(x,y) = 0$ for $x\neq y$, and therefore is not a $T2$-space.}
