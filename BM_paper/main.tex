...
TODO: the measurability issue arises when proving that the event that all processes are equal is measurable.
We need to prove that the event $A = \{\omega \mid \forall t \in T, \: Y_t^1(\omega) = Y_t^n(\omega)\}$ that two processes are equal is measurable.
In order to do so, we use continuity and the fact that the distance $d_E(Y^1_t(\omega), Y^n_t(\omega))$ between the two processes is a measurable function for all $t \in T$. This is in turn true because the pairs $(Y_t^1, Y_t^n)$ are measurable... which is something that requires more than what the theorem gives us.

TODO: the "limit of an indicator of the original process" condition.
