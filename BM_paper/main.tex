...
\end{itemize}
Note: we changed the proof from the paper (exponents in $\mathbb{N}$ and not $\mathbb{Z}$) to avoid formalization issues.

Note: no big correctness issues with the original paper, but they forgot the (easier) $p < 1$ case.
}

The Kolmogorov-Chentsov theorem gives the existence of a Hölder continuous version of a stochastic process, provided that the process satisfies a certain moment condition called the Kolmogorov condition.
...
A stochastic process indexed by $T$ with values in $E$ is a collection $(X_t)_{t \in T}$ of random variables $\Omega \to E$ \todorm{with measurability condition?}.

\todor{explain bounded covering number and Kolmogorov condition}

\begin{theorem}\label{thm:kolmogorov_chentsov_bound}
Suppose that $T$ has bounded internal covering number with constant $c>0$ and exponent $d > 0$.
...
\begin{align*}
  ...
  \: .
\end{align*}
If furthermore $E$ is a nonempty extended metric space ($d_E(x,y) = 0$ implies $x = y$) and is complete, and $T$ is second-countable, then the process $X$ has a version with almost surely Hölder continuous paths of exponent $\beta$ for all $\beta \in (0, (q - d)/p)$.
\end{theorem}
\todor{In the code we also have $E$ second-countable, but we are trying to remove it.}
\todor{Note: due to conventions and properties of the Kolmogorov condition, that makes sense also if $T'$ has zero diameter, in which case we get $0/0 = 0$.}
