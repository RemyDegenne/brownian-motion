...
\begin{definition}[\docs{ProbabilityTheory.IsBrownian}]\label{def:IsBrownian_preview}
...
Since Lemma~\ref{l:stetigcompact} gives the $\sigma$-additivity of an additive content \docs{MeasureTheory.projectiveFamilyContent}, which is defined through the projective family $P$, we have a proof of the Kolmogorov Extension Theorem \docs{MeasureTheory.projectiveLimitWithWeakestHypotheses}.
...
More precisely, the \mathlib lemma \docs{isCompact_iff_totallyBounded_isComplete}, which shows that a set $A \subseteq E$ is compact iff it is complete and totally bounded, requires $E$ to be a uniform space.
This is a weaker topological notion than a pseudo-metric space but in the case of a second-countable space (as is the case in our formalization), a uniform space can also be turned into a pseudo-metric space (\docs{UniformSpace.pseudoMetrizableSpace}).
%(recall that every pseudo-metric space is uniform, and a second-countable uniform space can be made into a pseudo-metric space).
