...
\begin{itemize}
\item $X_t$ has almost surely Hölder-continuous paths for any coefficient $\beta < \tfrac 12$.
...
\item For every almost surely finite stopping time $T$, the process $X_{T + t} - X_{T}$ is a Brownian motion independent of $\mathcal{F}_T$ (the $\sigma$-field of events determined by time $T$). In particular $X_t$ satisfies the strong Markov property.
