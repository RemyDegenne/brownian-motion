...
Let us describe briefly the Kolmogorov-Chentsov Theorem in a simple case here, before we dive deeper in the next sections. The result states that for a stochastic process $X$ (with $T = [0,1])$, assume we find $E, \beta, C > 0$ satisfying
\begin{align}
\label{eq:cs}
  \mathbf E[r(X_s, X_t)^E] \leq C|t-s|^{\beta + 1}, \qquad 0\leq s,t\leq 1.
\end{align}
Then there exists $Y = (Y_t)_{t\in [0,1]}$ with $\mathbf P(X_t = Y_t) = 1$ for all $t\in [0,1]$ and $Y$ has almost surely Hölder continuous paths with coefficient $\gamma$ for any $\gamma < \tfrac \beta E$.
...
We will use a more general version of this statement, replacing $T = [0,1]$ by a metric space with a property restricting the number of balls needed to cover $T$. This is based on recent work of \cite{talagrand2022upper} and \cite{kratschmer2023kolmogorov}.
...
It states that if $\mu$ is a Gaussian measure over a Banach space $E$, then there exists $E > 0$ such that
\begin{align*}
  \int_E \exp\left( E \|x\|^2 \right) \mu(\mathrm dx) < \infty.
\end{align*}
...
- chaining: sequence of points in covers at different scales

- the pair reduction lemma (to be used on a cover at a given scale)

general remark about chaining: we chose to follow \cite{kratschmer2023kolmogorov} partly because it uses chaining, which is a powerful technique that can be reused in other contexts (e.g. concentration inequalities for suprema of empirical processes).
}
...
Since $J \subseteq T$, it also has bounded covering number with the same exponent $d$ and a constant $2^d c$\todorm{ref}.
